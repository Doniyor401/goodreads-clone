...
class BookReviewAPITestCase(APITestCase):
    def setUp(self):
        self.user = CustomUser.objects.create(username="doniyor", first_name="Doniyor")
        self.user.set_password("somepass")
        self.user.save()
        self.client.login(username="doniyor", password="somepass")

    def test_book_review_detail(self):
        book = Book.objects.create(title="Book1", description="Description 1", isbn="12435232353")
        ...
        self.assertEqual(response.data['book']['isbn'], "12435232353")
        # user ga test
        self.assertEqual(response.data['user']['id'], self.user.id)
        self.assertEqual(response.data['user']['first_name'], "Doniyor")
        self.assertEqual(response.data['user']['username'], "doniyor")
